...
echo "GOVERSION = $GOVERSION" &>> /vagrant/bootstrap.log
...
if [ ! -d "/usr/local/go" ]; then
	echo "Installing the Go language compiler version $GOVERSION" &>> /vagrant/bootstrap.log
	wget -q https://storage.googleapis.com/golang/go$GOVERSION.linux-386.tar.gz 2>> /vagrant/bootstrap.log
	tar -C /usr/local -xzf go$GOVERSION.linux-386.tar.gz 2>> /vagrant/bootstrap.log
